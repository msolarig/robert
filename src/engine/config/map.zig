...
pub const Map = struct {
    ...
    // descriptive error.
    const MapError = error{ MapFileNotFound, MapInvalidJson, MapInvalidFormat, AutoFileNotFound, DatabaseFileNotFound, TableNotFound, IncorrectFeedMode, IncorrectExecMode };
    ...
    pub fn init(alloc: std.mem.Allocator, map_path: []const u8) MapError!Map {
        ...
        defer file.close();
        const json_bytes: []const u8 = file.readToEndAlloc(alloc, std.math.maxInt(usize)) catch |err| switch (err) {
            error.FileTooBig => return error.MapInvalidJson,
            else => return error.MapInvalidJson,
        };
        defer alloc.free(json_bytes);

        const MapPlaceHolder = struct { auto: []const u8, feed_mode: FeedMode, db: []const u8, table: []const u8, t0: u64, tn: u64, trail_size: u64, exec_mode: ExecMode };

        var parsed_map = std.json.parseFromSlice(MapPlaceHolder, alloc, json_bytes, .{}) catch return error.MapInvalidJson;
        defer parsed_map.deinit();

        return Map{
            .alloc = alloc,
            .auto = path_util.autoSrcRelPathToCompiledAbsPath(alloc, parsed_map.value.auto) catch return error.MapInvalidJson,
            .feed_mode = parsed_map.value.feed_mode,
            .db = path_util.dbRelPathToAbsPath(alloc, parsed_map.value.db) catch return error.MapInvalidJson,
            .table = alloc.dupe(u8, parsed_map.value.table) catch return error.MapInvalidJson,
